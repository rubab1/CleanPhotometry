...
def DoAll(filename='10_10_dol1/phot',filters=filters,AB_Vega=AB_Vega,
    ...
    input_data = [ascii.read(fileroot+filt+'_stips.txt',format='ipac') for filt in filters]

    output_data  = np.loadtxt(fileroot+filename)
    
    xy         = output_data[:,2:4].T
    Sky        = output_data[:,range(12,12+13*nfilt,13)].T
    ...
    all_in = lambda a,b: input_sources(input_data,a,b)
